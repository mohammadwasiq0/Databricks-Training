...
# MAGIC
# MAGIC #### Lab Notes
# MAGIC To successfully create an exploding join that spills to disk we had to specifically turn off the following optimizations:
# MAGIC - Predictive optimization ([AWS](https://docs.databricks.com/aws/en/optimizations/predictive-optimization) | [Azure](https://learn.microsoft.com/en-us/azure/databricks/optimizations/predictive-optimization) | [GCP](https://docs.databricks.com/gcp/en/optimizations/predictive-optimization)) for Unity Catalog managed tables in the **lab** schema.
...
# MAGIC   margin: 16px 0;
# MAGIC ">
# MAGIC   <strong style="display:block; color:#c62828; margin-bottom:6px; font-size: 1.1em;">Select Classic Compute</strong>
...
# MAGIC
# MAGIC Run the following cell to configure your working environment for this course. It will also set your perf_opt_lab catalog to your unique **labuser** catalog, and the default schema to **perf_opt_lab**. All tables will be read from and written to this location.
# MAGIC <br></br>
# MAGIC
...
# MAGIC
# MAGIC For more information, see Optimize performance with caching on Databricks:
...
# MAGIC Generate the **transactions** table with the schema below and write it to a table. This is the table with the largest amount of data, containing 2,000,000 rows.
# MAGIC
...
# MAGIC
# MAGIC - [spark.sql.autoBroadcastJoinThreshold](https://spark.apache.org/docs/latest/sql-performance-tuning.html#automatically-broadcasting-joins) documentation
# MAGIC
# MAGIC - [spark.databricks.adaptive.autoBroadcastJoinThreshold](https://spark.apache.org/docs/latest/sql-performance-tuning.html#converting-sort-merge-join-to-broadcast-join) documentation
...
# MAGIC
# MAGIC For more information, view the [spark.sql.shuffle.partitions](https://spark.apache.org/docs/latest/sql-performance-tuning.html#adaptive-query-execution) documentation.
# MAGIC
...
# MAGIC %skip
# MAGIC ## Rerun the cell to turn off the broadcast join features if not already turned off
# MAGIC spark.conf.set("spark.sql.autoBroadcastJoinThreshold", -1)
# MAGIC spark.conf.set("spark.databricks.adaptive.autoBroadcastJoinThreshold", -1)
# MAGIC
# MAGIC ## Deal with the spill by increasing the number of shuffle partitions to 8
# MAGIC ## Not a huge difference in this small example, but as the amount of data spilled increases
# MAGIC ## This can make a big difference
...
## Reset the shuffle.partitions to the default setting
...
# MAGIC %skip
# MAGIC spark.conf.unset("spark.sql.autoBroadcastJoinThreshold")
# MAGIC spark.conf.unset("spark.databricks.adaptive.autoBroadcastJoinThreshold")
# MAGIC
# MAGIC print(f'Default value of autoBroadcastJoinThreshold: {spark.conf.get("spark.sql.autoBroadcastJoinThreshold")}')
# MAGIC print(f'Default value of adaptive.autoBroadcastJoinThreshold: {spark.conf.get("spark.databricks.adaptive.autoBroadcastJoinThreshold")}')
...
# MAGIC
# MAGIC **HINT:** View the ANALYZE TABLE documentation for more information:
...
# MAGIC %skip
# MAGIC sql("ANALYZE TABLE transactions COMPUTE STATISTICS FOR COLUMNS country_id, store_id")
# MAGIC sql("ANALYZE TABLE stores COMPUTE STATISTICS FOR COLUMNS id")
# MAGIC sql("ANALYZE TABLE countries COMPUTE STATISTICS FOR COLUMNS id")
