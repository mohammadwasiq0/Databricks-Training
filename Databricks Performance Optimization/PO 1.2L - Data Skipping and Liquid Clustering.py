...
# MAGIC %md-sandbox
# MAGIC ## REQUIRED - SELECT A CLASSIC COMPUTE ENVIRONMENT
# MAGIC
# MAGIC <div style="
...
# MAGIC
# MAGIC Before executing cells in this notebook, please select your classic compute cluster in the lab. Be aware that **Serverless** is enabled by default. If you use Serverless, errors will be returned when setting compute runtime properties.Select the required compute environment listed below.
# MAGIC
# MAGIC - **Classic Compute (version = 15.4 LTS)**  
# MAGIC </br>
# MAGIC
# MAGIC <img src="./Includes/images/common/select-classic-compute.png" alt="Compute Select" width="400"/>
# MAGIC <br></br>
# MAGIC
...
# MAGIC
# MAGIC Run the following cell to configure your working environment for this course. It will also set your perf_opt_lab catalog to your unique **labuser** catalog, and the default schema to **perf_opt**. All tables will be read from and written to this location.
# MAGIC <br></br>
# MAGIC
...
# MAGIC %sql
# MAGIC USE CATALOG dbacademy_flightdata;
# MAGIC USE SCHEMA v01;
# MAGIC
...
# MAGIC %sql
# MAGIC SELECT 'flights', count(*) AS TotalRows
# MAGIC FROM flights
# MAGIC UNION ALL
# MAGIC SELECT 'flights_cluster_id', count(*) AS TotalRows
# MAGIC FROM flights_cluster_id
# MAGIC UNION ALL
# MAGIC SELECT 'flights_cluster_id_flightnum', count(*) AS TotalRows
# MAGIC FROM flights_cluster_id_flightnum;
...
# MAGIC
# MAGIC SELECT AVG(try_cast(ArrDelay AS DOUBLE))
# MAGIC FROM flights
# MAGIC WHERE UniqueCarrier = 'TW';
...
# MAGIC
# MAGIC 3. In the new window, locate the **Jobs** header at the top and click the number under **Associated SQL Query**.
# MAGIC
...
# MAGIC     **NOTE:** In the Vocareum lab environment if you click **View** without opening it in a new tab the pop up window will display an error.
# MAGIC
# MAGIC 3. In the new window, find the **SQL/DataFrame Properties*** header at the top and select the number.
# MAGIC
# MAGIC 4. Here you should see the entire query plan.
# MAGIC
# MAGIC 5. In the query plan, scroll down to the bottom and find **PhotonScan parquet dbacademy_flightdata.v01.flights (1)** and select the plus icon.
...
# MAGIC %sql
# MAGIC DESCRIBE HISTORY flights_cluster_id;
...
# MAGIC %sql
# MAGIC DESCRIBE TABLE EXTENDED flights_cluster_id;
...
# MAGIC %sql
# MAGIC DESCRIBE HISTORY flights_cluster_id_flightnum
...
# MAGIC %sql
# MAGIC DESCRIBE TABLE EXTENDED flights_cluster_id_flightnum;
...
# MAGIC | Metric    | Value    | Note    |
# MAGIC |-------------|-------------|-------------|
# MAGIC | cloud storage request count total (min, med, max)| 376 (3, 10, 14)| Refers to the number of requests made to the cloud storage systems like S3, Azure Blob, or Google Cloud Storage during job execution. This could involve multiple operations like reading metadata, accessing directories, or fetching the actual data.|
...
# MAGIC
# MAGIC <br></br>
# MAGIC ## Automatic Liquid Clustering (GA as of June 2025)
# MAGIC
# MAGIC ![LQ Auto](./Includes/images/Liquid-Clusters-OG.png)
# MAGIC
# MAGIC In the demos above, you manually selected clustering keys. **Automatic Liquid Clustering** eliminates this decision — Databricks analyzes your workload and selects the optimal keys for you.
# MAGIC
# MAGIC Powered by **Predictive Optimization**, it works by monitoring query predicates (`WHERE` clauses and `JOIN` filters), modeling which clustering keys would reduce the most data scanned, and only applying changes when the predicted performance gains outweigh the clustering cost. This runs asynchronously and **adapts over time** as query patterns change.
# MAGIC
# MAGIC Databricks recommends it for **all Unity Catalog managed tables** — especially when you're unsure which columns to cluster by, query patterns evolve, or you want a hands-off approach across many tables. It requires **DBR 15.4 LTS+**, **Unity Catalog managed Delta tables**, and **Predictive Optimization enabled** on the catalog or schema.
# MAGIC
# MAGIC #### References
# MAGIC - [Announcing Automatic Liquid Clustering](https://www.databricks.com/blog/announcing-automatic-liquid-clustering) (Blog)
# MAGIC - Documentation:
# MAGIC [AWS](https://docs.databricks.com/aws/en/delta/clustering#automatic-liquid-clustering) |
# MAGIC [Azure](https://learn.microsoft.com/en-us/azure/databricks/delta/clustering#automatic-liquid-clustering) |
# MAGIC [GCP](https://docs.databricks.com/gcp/en/delta/clustering#automatic-liquid-clustering)
# MAGIC

# COMMAND ----------

# MAGIC %md
# MAGIC #### Enabling Automatic Liquid Clustering
# MAGIC
# MAGIC Use `CLUSTER BY AUTO` when creating a new table or altering an existing one. Run `DESCRIBE TABLE EXTENDED` to confirm it's enabled — look for `clusterByAuto = true` in the table properties.
...
# MAGIC %sql
# MAGIC -- Enable on a new table
# MAGIC CREATE OR REPLACE TABLE auto_clustered_example (
# MAGIC   id BIGINT,
...
# MAGIC
# MAGIC -- Or enable on an existing unpartitioned / liquid clustered table
# MAGIC -- ALTER TABLE my_existing_table CLUSTER BY AUTO;

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Verify: look for "clusterByAuto = true" and check clusteringColumns for selected keys
# MAGIC DESCRIBE TABLE EXTENDED auto_clustered_example;
...
# MAGIC     Why You May Not See Clustering Keys Selected Immediately
# MAGIC   </strong>
# MAGIC   <div style="color:#333;">
# MAGIC
# MAGIC After enabling `CLUSTER BY AUTO`, you will **not** see clustering keys appear right away. Predictive Optimization needs to accumulate **sufficient query history** on the table — it analyzes historical query scan statistics to determine which columns are worth clustering by. Key selection runs as a background maintenance operation on its own schedule, not at query time.
# MAGIC
# MAGIC You may not see keys selected if the table is too small, has too few queries, lacks repeated filter patterns, or if the cost-benefit analysis determines that clustering would not provide meaningful improvement. In a live class environment, this means the `clusteringColumns` field in `DESCRIBE TABLE EXTENDED` will likely remain empty during the demo. This is expected behavior — Automatic Liquid Clustering is designed for production workloads that run over days and weeks, not for short-lived demo tables.
# MAGIC
# MAGIC   </div>
# MAGIC </div>
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS auto_clustered_example;
