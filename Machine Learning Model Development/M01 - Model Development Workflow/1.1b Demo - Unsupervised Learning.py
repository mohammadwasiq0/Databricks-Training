...
cluster_data = []
for i, (cluster_id, unique_id) in enumerate(zip(cluster_labels, news_df.select("unique_id").rdd.map(lambda x: x[0]).collect())):
    cluster_data.append((unique_id, int(cluster_id)))

clusters_spark_df = spark.createDataFrame(cluster_data, ["unique_id", "Cluster"])
clusters_spark_df.cache()
